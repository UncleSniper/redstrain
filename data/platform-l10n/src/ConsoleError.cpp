#include <redstrain/platform/ConsoleError.hpp>
...
namespace platform {
	...
	REDSTRAIN_LOCALIZE_ERROR16(ConsoleError) {
		...
		switch(error.getOperation()) {
			#define clamp(constant) \
				case ConsoleError::constant: \
					opkey = l10n::MSG_CONSOLE_ERROR_ ## constant; \
					break;
			clamp(CLEAR)
			clamp(SET_CURSOR_POSITION)
			clamp(SET_TEXT_ATTRIBUTE)
			#undef clamp
			default:
				opkey = l10n::MSG_CONSOLE_ERROR_UNKNOWN_OPERATION;
				break;
		}
		out << catalog.formatMessage(
			locale, l10n::MSG_CONSOLE_ERROR,
